...
<meta name="theme-color" content="#ffffff">');
	...
	public function addHeader(string $header = '') : bool
		...
		$menu->addMenuItem(new \PHPFUI\MenuItem('Home', '/BuriedTreasure'));
		$menu->addMenuItem(new \PHPFUI\MenuItem('Shows', '/BuriedTreasure/Shows/info/0'));
		$menu->addMenuItem(new \PHPFUI\MenuItem('Artists', '/BuriedTreasure/Artists'));
		$menu->addMenuItem(new \PHPFUI\MenuItem('Titles', '/BuriedTreasure/Titles'));
		$menu->addMenuItem(new \PHPFUI\MenuItem('Albums', '/BuriedTreasure/Albums'));
		$menu->addMenuItem(new \PHPFUI\MenuItem('Download', '/BuriedTreasure/Download'));
		$settings = new \App\Settings\Admin();

		if ($settings->allowAdmin)
			{
			$menu->addMenuItem(new \PHPFUI\MenuItem('Admin', '/BuriedTreasure/Admin'));
			}
		...
		$mdPath = PROJECT_ROOT . '/BuriedTreasureDocs/' . $header . '.md';
	...
	public function getRelativeURL(string $lastPart) : string
		...
		return '/BuriedTreasure/' . \implode('/', $parts);
		}
